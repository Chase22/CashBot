...
public class SetAmountPic extends ConfigCommand {
    ...
    public SetAmountPic(final CashChatService cashChatService) {
    ...
    protected Optional<CashBotReply> executeCommand(final AbsSender absSender, Message message, final String[] arguments) {
        ...
        cashChat.setAmountPic(Integer.parseInt(arguments[0]));
        cashChatService.save(cashChat);

        return Optional.of(new CashBotReply(message.getChatId(), "Amount set to %s", cashChat.getAmountPic()));
    }
}